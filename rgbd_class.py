...
class ARID_Dataset(object):
    def __init__(self, root, transforms=None):
        ...
        self.dict_scenes['path_rgb'] = list()
        self.dict_scenes['path_depth'] = list()
        ...
        for direc in os.listdir(self.root):
            for subdirec in os.listdir(osp.join(self.root+direc)):
                if subdirec == 'rgb' or subdirec == 'depth':
                    ...
                    for img in images:
                        if img != 'crops' and img != 'crops_processed':
                            ...
                            if subdirec == 'rgb':
                                self.dict_scenes['path_rgb'].append(path)
                            else:
                                self.dict_scenes['path_depth'].append(path)
                else: #json
    ...
    def __getitem__(self, idx):
        ...
        rgb = mpimg.imread(self.dict_scenes['path_rgb'][idx])
        depth = mpimg.imread(self.dict_scenes['path_depth'][idx])
        ...
        rgbd = transforms.ToPILImage()(rgbd_tensor)
        img = transforms.ToTensor()(rgbd)
